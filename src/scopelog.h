...
//
//	既定は ON (main.cpp の setup で有効化。SETUP 画面で切れる)。
...
//	    <mag_min> <on_cnt> <off_cnt> <real_cnt> <hops>
//	                                           スコープ列ごと (t_ms はサンプル数由来)。
//	                                           side/near/limit/nf はトーン判定に使われた値で、
//	                                           ON: env > 0.6*limit かつ env > side*3 かつ env > near*2
//	                                           OFF: env < 0.4*limit または env < side*2.5 または env < near*3.2
//	                                           on/off/real_cnt はブロック単位の実際の判定結果
//	                                           (列単位の env と side/near の比からは再現できないため)
