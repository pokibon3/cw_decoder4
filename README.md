...
現在のバージョンは `v3.1` です (`src/version.h` の `FW_VERSION`)。
...
- 符号内/文字間の境界はキーイングの重み (符号内スペース長 ÷ 短点長) に追従する。
  ...
  境界は「実測した符号内スペース長」と「あるべき文字間 (3 単位)」の中点に置き、
  重みは短点長との比 (0.8〜2.0 に拘束) で持つ。上限 2.0 でも境界は 2.5 単位にしかならず、
  3 単位の文字間を飲み込む正帰還には構造上入らない
