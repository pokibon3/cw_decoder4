...
現在のバージョンは `v2.6` です (`src/version.h` の `FW_VERSION`)。
...
タッチ座標がズレる場合は SETUP の **タッチ調整** で合わせてください (結果は NVS に保存)。
...
夏時間は SETUP の `Summer Time` が ON のときだけ適用します (設定は NVS に保存)。

デコーダ画面のステータス行にある `SETUP` ボタンから次の操作ができます。
項目は 2 列 5 行に並びます
(WiFi設定 / 時刻合わせ / 夏時間 / スコープ / タッチ / FW更新 / 自動更新 / 初期化 / About)。

- **時刻合わせ**: ▲▼ で年月日時分を合わせて `決定` (RTC は無いので電源を切るとビルド日時に戻ります)
- **自動更新** (既定 ON): 起動時 (スプラッシュの直後) に WiFi へ数秒だけ接続し、**NTP 時刻同期と
  ファームウェアの新版チェックを 1 回の接続でまとめて** 行います。新版があれば画面に
  `新しいファームウェアがあります v2.5 → v2.6` と出るので、`更新` / `キャンセル` を選びます。
...
- **Summer Time**: 世界時計の夏時間を使うか (ON のとき各国の規則で自動計算)
- **タッチ調整**: 縦横 1/8 だけ内側に出る十字を4点タップして座標を校正します (端はベゼルが近く狙いにくいので
...
- **CW Decoder について**: バージョン / ビルド日時 / パネル / 起動領域 / WiFi / NTP 同期状況を表示
...
NTP 時刻同期 (WiFi を登録した場合) は **起動時 (自動更新が ON のとき)** と、
...
時計 / NTP / WiFi / OTA / タッチ調整 / スコープログは Web では意味がないので
