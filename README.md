...
現在のバージョンは `v2.5` です (`src/version.h` の `FW_VERSION`)。
...
(WiFi設定 / 時刻合わせ / Summer Time / スコープログ / タッチ調整 / ファームウェアアップデート / 初期化 / About)。
...
- **タッチ調整**: 縦横 1/8 だけ内側に出る十字を4点タップして座標を校正します (端はベゼルが近く狙いにくいので
  ...
  **ズレた値を保存してしまったときの復帰**: `BOOT` ボタンを押したまま電源を入れ、そのまま 2 秒押し続けると
  校正値を消して既定値に戻します (タッチを一切使いません)。この案内は調整済みのとき起動画面にも出ます。
