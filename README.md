...
- 下 1/3 左: FFT スペクトラム (ヘッダに入力レベルメーター: 緑 <55% / 黄 / 赤 ≥75%、レール到達で `CLIP` 表示) (約300〜1200Hz、1bin=31.25Hz のライン表示、ピークホールド、TONE 選択レンジ 600〜1000Hz のガイド帯 + 選択トーンの検出帯域(±34Hz)表示)
...
ESP32 の ADC は約 2.45V 以上で非線形になるため、FFT パネルの入力レベルメーターが
**緑の範囲 (55% 未満) に収まるレベル**で使ってください。黄〜赤 (圧縮歪み) や `CLIP` 点灯
(レール到達) の状態ではサイドビンが上がってデコード率が落ちます。
